...
def mnemonic_to_eth(mw):
    private_key = mnemonic_to_private_key(mw,
            str_derivation_path=f'{ETH_DERIVATION_PATH}/0')
    public_key = PublicKey(private_key)
    return{"private_key":binascii.hexlify(private_key).decode("utf-8"),"public_key":binascii.hexlify(bytes(public_key)).decode("utf-8"),"address":public_key.address()}

if __name__ == '__main__':
    import sys
    # with open(sys.argv[1], 'r') as f:
    #     mnemonic = ' '.join(f.read().split())
    mnemonic ="inside vendor knock science green mother course soda portion solution morning option"
